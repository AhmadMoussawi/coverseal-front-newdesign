...
const cloudFront = new CloudFrontClient.Signer(
...
export default async function handler(req, res) {
  ...
  /*var unsigned_url  = req.query.unsigned_url;
  if (!unsigned_url) {
    res.status(400).json({
      success: false,
      message: "unsigned_url is required",
    });
  }*/
    ...
    const policy = JSON.stringify(
      {
         Statement: [
            {
               Resource: req.query.unsigned_url,
               Condition: {
                  DateLessThan: {
